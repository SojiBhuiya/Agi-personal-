...
* tag/title: `v<versionName>` / `AGI Assistant <versionName>` — both read from the built APK
  ...
  a pushed tag that does not match the APK version fails the job. Nothing is hard-coded.
* assets: `agi-assistant-<versionName>-release.apk` and `agi-assistant-<versionName>-release.apk.sha256`
  (sha256sum format);
* notes: generated by `scripts/release_notes.sh` from the matching `## <versionName>` section of
  `CHANGELOG.md`, plus the markers the parser reads: `versionCode: N`, `sha256: <hex>` and a
  `<hex>  <apk>` line;
* if the release already exists the notes are refreshed and the assets replaced (`--clobber`), so the
  release always carries the newest signed build of that version;
* the step then reads `/releases/latest` back, requires it to be this tag with both assets uploaded,
