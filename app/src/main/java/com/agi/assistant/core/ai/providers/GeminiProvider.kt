...
class GeminiProvider(private val config: ProviderConfig) : AiProvider {
    ...
    override suspend fun complete(request: AiRequest): AiResponse {
        ...
        val url = config.baseUrl.trimEnd('/') + "/v1beta/models/${config.model}:generateContent?key=${config.apiKey}"
        val json = HttpJson.post(url, body, emptyMap(), config.timeoutMs)
        val candidate = json.optJSONArray("candidates")?.optJSONObject(0)
            ?: throw AiProviderException("Gemini returned no candidates: ${json.toString().take(200)}")
